//------------------------------------------------------------------------------
// <auto-generated>
//     這個程式碼是由範本產生。
//
//     對這個檔案進行手動變更可能導致您的應用程式產生未預期的行為。
//     如果重新產生程式碼，將會覆寫對這個檔案的手動變更。
// </auto-generated>
//------------------------------------------------------------------------------

namespace MVC_HW3.Models
{
    using System;
    using System.Collections.Generic;
    
    public partial class tTravelCase
    {
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2214:DoNotCallOverridableMethodsInConstructors")]
        public tTravelCase()
        {
            this.tAlbum = new HashSet<tAlbum>();
            this.tDayTravel = new HashSet<tDayTravel>();
            this.tPenalty = new HashSet<tPenalty>();
            this.tRegistrationDetail = new HashSet<tRegistrationDetail>();
            this.tRegistrationOpen = new HashSet<tRegistrationOpen>();
        }
    
        public int fTC_ID { get; set; }
        public string fTC_Title { get; set; }
        public int fTC_Top { get; set; }
        public int fTC_Gate { get; set; }
        public decimal fTC_Cost { get; set; }
        public string fTC_TDate { get; set; }
        public string fTC_Content { get; set; }
        public string fTC_Notes { get; set; }
        public int fTC_LorD { get; set; }
        public byte[] fTC_Picture { get; set; }
        public int fCC_ID { get; set; }
        public int fTC_PerL { get; set; }
        public decimal fTC_Car { get; set; }
        public decimal fTC_Eat { get; set; }
        public Nullable<decimal> fTC_hotel { get; set; }
    
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2227:CollectionPropertiesShouldBeReadOnly")]
        public virtual ICollection<tAlbum> tAlbum { get; set; }
        public virtual tCityCountry tCityCountry { get; set; }
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2227:CollectionPropertiesShouldBeReadOnly")]
        public virtual ICollection<tDayTravel> tDayTravel { get; set; }
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2227:CollectionPropertiesShouldBeReadOnly")]
        public virtual ICollection<tPenalty> tPenalty { get; set; }
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2227:CollectionPropertiesShouldBeReadOnly")]
        public virtual ICollection<tRegistrationDetail> tRegistrationDetail { get; set; }
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2227:CollectionPropertiesShouldBeReadOnly")]
        public virtual ICollection<tRegistrationOpen> tRegistrationOpen { get; set; }
    }
}
