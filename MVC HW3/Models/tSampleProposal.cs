//------------------------------------------------------------------------------
// <auto-generated>
//     這個程式碼是由範本產生。
//
//     對這個檔案進行手動變更可能導致您的應用程式產生未預期的行為。
//     如果重新產生程式碼，將會覆寫對這個檔案的手動變更。
// </auto-generated>
//------------------------------------------------------------------------------

namespace MVC_HW3.Models
{
    using System;
    using System.Collections.Generic;
    
    public partial class tSampleProposal
    {
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2214:DoNotCallOverridableMethodsInConstructors")]
        public tSampleProposal()
        {
            this.tActivitySelect = new HashSet<tActivitySelect>();
        }
    
        public int fSP_ID { get; set; }
        public Nullable<int> fEp_ID { get; set; }
        public Nullable<System.DateTime> fSP_Date { get; set; }
        public Nullable<int> fCC_ID { get; set; }
        public string fSP_Attraction { get; set; }
    
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2227:CollectionPropertiesShouldBeReadOnly")]
        public virtual ICollection<tActivitySelect> tActivitySelect { get; set; }
        public virtual tCityCountry tCityCountry { get; set; }
        public virtual tEmployee tEmployee { get; set; }
    }
}
