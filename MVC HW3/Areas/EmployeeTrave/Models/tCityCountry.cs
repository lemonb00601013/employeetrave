//------------------------------------------------------------------------------
// <auto-generated>
//     這個程式碼是由範本產生。
//
//     對這個檔案進行手動變更可能導致您的應用程式產生未預期的行為。
//     如果重新產生程式碼，將會覆寫對這個檔案的手動變更。
// </auto-generated>
//------------------------------------------------------------------------------

namespace MVC_HW3.Areas.EmployeeTrave.Models
{
    using System;
    using System.Collections.Generic;
    
    public partial class tCityCountry
    {
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2214:DoNotCallOverridableMethodsInConstructors")]
        public tCityCountry()
        {
            this.tCityCountry1 = new HashSet<tCityCountry>();
            this.tTravelCase = new HashSet<tTravelCase>();
        }
    
        public int fCC_ID { get; set; }
        public string fCC_Place { get; set; }
        public Nullable<int> fCC_Dad { get; set; }
    
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2227:CollectionPropertiesShouldBeReadOnly")]
        public virtual ICollection<tCityCountry> tCityCountry1 { get; set; }
        public virtual tCityCountry tCityCountry2 { get; set; }
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2227:CollectionPropertiesShouldBeReadOnly")]
        public virtual ICollection<tTravelCase> tTravelCase { get; set; }
    }
}
